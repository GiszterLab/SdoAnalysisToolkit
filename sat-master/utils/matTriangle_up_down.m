%% Matrix Triangle Up/Down (Sum, Difference)
...
% OUTPUT: 
%   val: Total absolute magnitude of off-diagonal elements for each
%   page of M. 

% combined sum/difference into a single script; 
...
function [netSum] = matTriangle_up_down(M, OPERATION)
...
switch OPERATION
    case {'sum', 'Sum'}
        func = @(upX,dnX) sum(abs(upX)+abs(dnX)); 
    case {'difference', 'Difference'}
        func = @(upX, dnX) sum(upX-dnX); 
end
