...
for s = 1:n_sFields %For each test
    ...
    if GROUPWISE
        ...
        for h = 1:n_hh
            ...
            if ~SKIP_MULTICOMP
                try
                    if verLessThan('matlab', '9.12') % 9.12 = R2022a
                        str = evalc('multicompare(stats, "CType", "bonferroni")'); 
                    else
                        str = evalc('multicompare(stats, "CriticalValueType", "bonferroni")'); 
                    end
                    mc{h} = ans;
                catch ME
                    if verLessThan('matlab', '9.12')
                        mc{h} = multcompare(stats, 'CType', 'bonferroni');
                    else
                        mc{h} = multcompare(stats, 'CriticalValueType', 'bonferroni');
                    end   
                    %warning('multcompare failed for hypothesis %d: %s', h, ME.message);
                    %mc{h} = [];
                end
            end
        end
        ...
        try
            ...
            if verLessThan('matlab', '9.12')
                mc = multcompare(stats, 'CType', 'bonferroni');
            else
                mc = multcompare(stats, 'CriticalValueType', 'bonferroni');
            end
