...
%%
function [sdo] = populateSDOArray4_lowMemory(xtdc, ppdc, vars)
arguments
    xtdc xtDataCell
    ppdc ppDataCell
    vars.pxFilter       = [1,1]; 
...
%// Up to this point, the calculations are still quite fast. 
...
for m = 1:N_XT_CHANNELS
    ...
    for tr = 1:N_TRIALS
        ...
        %
        trLen = length(xtdc.data{1,tr}(m).stateSignal); 
        bkgndLen = trLen;
        ...
        bck_xv0     = xtdc.getValuesAtIndices(rand_idx, 'useChannels', m, "dataField","stateSignal", 'useTrials', tr);  
        bck_xv1     = xtdc.getValuesAtIndices(rand_idx, 'usechannels', m, "dataField","stateSignal", 'useTrials', tr); 
        %
        [pxt0_Bkgd, pxt1_Bkgd] = pxTools.get_pxt_core(bck_xv0{1}, bck_xv1{1}, N_BINS, PX_FSM_WID, PX_FSM_STD,1); 
        ...
        switch vars.method
            ...
            case 'asymmetric'
                [bkgdTrDeltaSDO, bkgdTrJointSDO] = SAT.compute.sdo5(pxt0_Bkgd, pxt1_Bkgd, 0, ...
...
%}
    ...
    for u = 1:N_PP_CHANNELS
        ...
        for tr = 1:N_TRIALS
            ...
            [tr_shuff_u_xv0, tr_shuff_u_xv1] = ppdc.getPerieventIndices(tr,u,"dataField","shuffle", ...
                'fs', xtdc.fs, 'n_shift', PX_NSHIFT, 'z_delay', PX_ZDELAY, ...
                't0_nPoints', N_PX0_PTS, 't1_nPoints',N_PX1_PTS); 

            %___________
            %// We'll have to take the shuffles 1 by 1, even if slower
            for ss = 1:N_SHUFF
                %
                ss_xv0     = xtdc.getValuesAtIndices(tr_shuff_u_xv0{1}(:,:,ss), 'useChannels', m, "dataField","stateSignal", 'useTrials', tr);  
                ss_xv1     = xtdc.getValuesAtIndices(tr_shuff_u_xv1{1}(:,:,ss), 'usechannels', m, "dataField","stateSignal", 'useTrials', tr); 
                % __ Filter __
                [pxt0_ss, pxt1_ss] = pxTools.get_pxt_core(ss_xv0{1}, ss_xv1{1}, N_BINS, PX_FSM_WID, PX_FSM_STD,1); 
                %
                if vars.backgroundSubtraction == 1

                    %// Potentially a weakness here if there is a drift in
                    %background over time
                    L0 = SAT.sdoUtils.normsdo(bkgdDeltaSDO, bkgdJointSDO);
                    %
                    L0pdPx1 = L0*pxt0_ss+pxt0_ss; 
                    %
                    pxt0_ss = L0pdPx1; % OVERRIDE
                end
                %___________________
                switch vars.method
    
                    case {'original', 'optimized'} %'original'
                        [trShuffDeltaSDO, trShuffJointSDO] = SAT.compute.sdo3(pxt0_ss, pxt1_ss, 0, ...
                            "parallelCompute",vars.parallelCompute, rescale=false); 
                    case 'asymmetric'
                        [trShuffDeltaSDO, trShuffJointSDO] = SAT.compute.sdo5(pxt0_ss, pxt_ss, 0, ...
                            'asymmetry', asymmetry_type, ... 
                            "parallelCompute",vars.parallelCompute, rescale=false);
                        %{
                    case 'optimized'
                ...
                %________
                unitShuffDeltaSDO(:,:,ss) = unitShuffDeltaSDO(:,:,ss) + trShuffDeltaSDO; 
                unitShuffJointSDO(:,:,ss) = unitShuffJointSDO(:,:,ss) + trShuffJointSDO; 
            end

            %

            [obs_idx0, obs_idx1]    = ppdc.getPerieventIndices(tr, u,'fs', ...
                xtdc.fs, 'n_shift',PX_NSHIFT,'t0_nPoints', N_PX0_PTS, ...
                't1_nPoints',N_PX1_PTS, 'z_delay', PX_ZDELAY); 

            %
            unit_xv0     = xtdc.getValuesAtIndices(obs_idx0{1}, 'useChannels', m, "dataField","stateSignal", 'useTrials', tr);  
            unit_xv1     = xtdc.getValuesAtIndices(obs_idx1{1}, 'usechannels', m, "dataField","stateSignal", 'useTrials', tr); 
            %
            [pxt0_obs, pxt1_obs] = pxTools.get_pxt_core(unit_xv0{1}, unit_xv1{1}, N_BINS, PX_FSM_WID, PX_FSM_STD,1); 
        ...
        %}
            ...
            if vars.backgroundSubtraction == 1
                ...
                L0pdPx1_u = L0*pxt0_obs+pxt0_obs; 
            ...
            switch vars.method
                case 'original'
                    [trDeltaSDO, trJointSDO] = SAT.compute.sdo3(pxt0_obs,pxt1_obs, 0, ...
                        parallelCompute=vars.parallelCompute, rescale=false); 
                case 'asymmetric'
                    [trDeltaSDO, trJointSDO] = SAT.compute.sdo5(pxt0_obs,pxt1_obs, 0, ...
                        'asymmetry', asymmetry_type, ... 
                        parallelCompute=vars.parallelCompute, rescale=false);
                case 'optimized'
                    [trDeltaSDO, trJointSDO] = SAT.compute.sdo7(pxt0_obs,pxt1_obs, 0, ...
                        parallelCompute=vars.parallelCompute, rescale=false); 
            end     
            %{
            switch vars.method
